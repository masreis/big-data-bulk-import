...
public class InsertBolsaFamilia {
	private static Logger logger =
			Logger.getLogger(InsertBolsaFamilia.class);
	...
	public void inserir(String filename) {
		...
		String cql = "INSERT INTO  %s.%s (ID, UF,"
				+ "CODIGO_MUNICIPIO, NOME_MUNICIPIO,"
				+ "NOME_BENEFICIARIO, VALOR_PAGO," + "MES_ANO)"
				+ "VALUES (?, ?, ?, ?, ?, ?, ?)";
		cluster =
				Cluster.builder().addContactPoints(contactPoints)
						.withPort(port).build();
		...
		try (BufferedReader reader =
			...
			PreparedStatement pstmt = session.prepare(
					String.format(cql, "scalability", "bfs"));
